...
export default function EditGalleryPage() {
  ...
  useEffect(() => {
    const fetchGallery = async () => {
      try {
        const data = await apiRequest.get(`/gallery/${id}`);
        if (data.success && data.gallery) {
          ...
          setFormData({
            ...
            category: g.category || "Residential",
            clientName: g.clientName || "",
            location: g.location || "",
            completionDate: g.completionDate || "",
            carpetType: g.carpetType || "",
            areaCovered: g.areaCovered || "",
            designer: g.designer || "",
            technique: g.technique || "",
            materials: Array.isArray(g.materials) ? g.materials.join(", ") : "",
          ...
          setImages(g.images || []);
          setFeaturedImage(g.featuredImage || "");
        }
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await apiRequest.patch(`/gallery/${id}`, payload);
      if (data.success) {
  ...
  if (loading) {
    return (
      <AdminLayout>
        <div style={{ color: "#fff", padding: "40px", textAlign: "center" }}>Loading project...</div>
      </AdminLayout>
    );
  }
    ...
    <AdminLayout>
      <div style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <div>
            <h1 style={styles.title}>Edit Project: {formData.title}</h1>
            <p style={styles.subtitle}>Modify your portfolio entry</p>
          </div>
        </div>
