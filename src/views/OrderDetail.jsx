...
export default function OrderDetail() {
  ...
  const handleStatusUpdate = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setTrackingNumber("");
        setCarrier("");
        fetchOrderDetail(); // Refresh full data
      }
    } catch (err) {
  ...
  const handleAddNote = async (e) => {
    ...
    setAddingNote(true);
    try {
      const data = await apiRequest.post(`/orders/${id}/notes`, {
        note: newNote,
      });
      if (data.success) {
        setNewNote("");
          ...
          <div style={styles.panel} className="glass">
                ...
                <div key={item.id || item._id} style={styles.itemRow}>
                  <div style={styles.itemImageContainer}>
                    {item.image ? (
                      ...
                      <div style={styles.itemImageFallback}>No Image</div>
                    )}
                  </div>
            ...
            <div style={styles.breakdown}>
              ...
              {order.couponDiscount > 0 && (
                <div style={{ ...styles.breakdownRow, color: "var(--color-success)" }}>
              ...
              {order.timeline?.map((evt, idx) => (
                <div key={evt.id || idx} style={styles.timelineItem}>
                  ...
                  <div style={styles.timelineContent}>
                    ...
                    {evt.updatedBy && (
                      <span style={styles.timelineAuthor}>
                        Updated by: {evt.updatedBy.firstName} {evt.updatedBy.lastName}
                      </span>
                    )}
                  </div>
                </div>
              ...
              <form onSubmit={handleStatusUpdate} style={styles.statusForm}>
                ...
                {newStatus === "shipped" && (
                  <div style={styles.shippingInputs} className="fade-in">
                    <div className="form-group">
                      <label className="form-label" htmlFor="carrierInput">Shipping Carrier</label>
                      <input
                        id="carrierInput"
                        type="text"
                    ...
                    <div className="form-group">
                      <label className="form-label" htmlFor="trackingInput">Tracking Number</label>
                      <input
                        id="trackingInput"
                        type="text"
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="statusNote">Fulfillment Note (Optional)</label>
                  <textarea
                    id="statusNote"
                    placeholder="Describe status change context..."
                    value={statusNote}
          ...
          <div style={styles.panel} className="glass">
            ...
            <div style={styles.notesContainer}>
              ...
              <form onSubmit={handleAddNote} style={styles.noteForm}>
                <textarea
                  placeholder="Record an internal comment..."
                  value={newNote}
                  ...
                  {order.refunds.map((ref) => (
                    <div key={ref._id} style={styles.refundRequestItem}>
                      <div style={styles.refundHeader}>
                        ...
                        <span className={`badge badge-${
                          ref.status === "processed" ? "success" :
                          ref.status === "requested" ? "warning" : "danger"
                        }`}>
                          {ref.status}
                        </span>
                      </div>
                      ...
                      {ref.status === "requested" && (
                        <div style={styles.refundActionsForm}>
                          ...
                          <div style={styles.refundButtonRow}>
                            <button
                              onClick={() => handleProcessRefundRequest(ref._id, "approve")}
                              className="btn btn-primary"
                              style={styles.actionRefundBtn}
                              disabled={processingRefund}
                            >
                              Approve
                            </button>
                            <button
                              onClick={() => handleProcessRefundRequest(ref._id, "reject")}
                              className="btn btn-secondary"
                              style={{ ...styles.actionRefundBtn, color: "var(--color-danger)" }}
                              disabled={processingRefund}
                            >
                              Reject
                            </button>
                ...
                <form onSubmit={handleRequestRefund} style={styles.refundRequestForm}>
                  ...
                  <div className="form-group">
                    ...
                    <input
                      id="refundReason"
                      type="text"
                      placeholder="Customer returned items..."
                      value={refundReason}
...
const styles = {
  ...
  timelineDot: {
    ...
    boxShadow: "0 0 6px var(--primary-brand-glow)",
    marginTop: "6px",
  },
  timelineLine: {
    width: "2px",
